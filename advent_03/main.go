...
func advent01() []int {
	minDist, totalSteps := math.MaxInt64, math.MaxInt64
	grid := map[Point][]int{}
	for li, l := range strings.Split(input, "\n") {
		step, x, y := 1, 0, 0
			...
			for i := 0; i < steps; i++ {
				...
				if stepsToReach[li] == 0 {
					...
					if stepsToReach[0] > 0 && stepsToReach[1] > 0 {
						minDist = min(minDist, mdFromCenter(x, y))
						totalSteps = min(totalSteps, stepsToReach[0]+stepsToReach[1])
					}
				}
				step = step + 1
			}
...
//	arr1 := stringConvert(wires[0])
//	arr2 := stringConvert(wires[1])
//	p1 := Point{0, 0}
//
//	for _, v := range arr1 {
//		direction := v[:1]
//		movements, _ := strconv.Atoi(v[1:])
//
//		switch direction {
//		case "D":
//			p1.x = p1.x - movements
...
//}
//
//func stringConvert(x string) []string {
//	a := strings.Split(x, ",")
//	b := make([]string, len(a))
//
//	for i, v := range a {
